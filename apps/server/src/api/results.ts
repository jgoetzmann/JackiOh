 ...
 */
async function writeResult(deps: ServerDeps, input: WriteInput): Promise<ResultRow> {
  return deps.store.tx(async (t: Store) => {
...
export function createRecordResult(deps: ServerDeps): RecordResult {
  return (input: RecordResultInput) => writeResult(deps, { ...input, ratingPolicy: "elo" });
}
