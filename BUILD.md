...
| `burned` | Card rises face-up over the full hand under a "Hand full" tag, catches fire and burns away toward the graveyard (a back when the viewer reads the sentinel, R317) | 700 ms | the hand's notice reads "Hand full" and names the card during the burn, then GY counter increments | `burn`: fire and embers over the hand, then embers at the graveyard |
| `fatigue` | Library pile shakes and dims as it comes up empty and a "Fatigue N" badge rises from it; the `damage` row after it lands the hit on the hero | 600 ms | the library pile gains `data-animating="fatigue"` and its notice reads "Fatigue N", N the event's `count` (R315, R318) | `fatigue`: dust puffs out of the empty pile and void wisps streak from it to the owner's hero |
| `libraryOverflow` | A "Library full" tag flashes on the library pile and the refused card bounces off it, then fizzles to smoke (`notCreated`, `ceased`) or drops toward the graveyard (`graveyard`) | 500 ms | the library pile gains `data-animating="libraryOverflow"`, its notice reads "Library full" and shows the card's face, or a back for the sentinel (R316, R318) | `overflow`: a refusal ring on the pile, then a smoke fizzle or embers at the graveyard |
