...
| A11 | Deckbuilder testids, which BUILD names none of: `card-pool-<cardId>`, `deck-tab-<n>`, `deck-drop-<n>`, `deck-<n>-card-<cardId>` (`n` 1-based, as the screen labels the decks), and a drag carrying the catalog id on `application/x-jackioh-card` plus `text/plain`. `cy.dragCardToDeck` is written against exactly these. | `support/testids.ts`, `support/commands.ts` | `apps/web` |
...
| `dragCardToDeck(cardId, deckIndex)` | A11's whole gesture — `dragstart`, `dragover`, `drop`, `dragend`, one `DataTransfer` built in the app's window. |
