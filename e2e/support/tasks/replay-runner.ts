...
//
// Reads {seed, decks, log, state} as JSON on stdin and prints one JSON line:
...
type Payload = { seed: string; decks: [string[], string[]]; log: unknown[]; state: unknown };
...
async function main(): Promise<void> {
  ...
  const engine = (await import("../../../packages/engine/src/replay.ts")) as {
    fold: (input: { seed: string; decks: [string[], string[]]; log: unknown[]; catalog?: unknown }) => {
      state: unknown;
      errors: unknown[];
    };
