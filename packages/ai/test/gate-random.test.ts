...
//
// The AI on Easy at AI_GATE_BUDGET must win at least AI_GATE.minWinRate["ai-vs-random"] of its
// games, seats alternating. `pnpm test` plays the smoke size (AI_GATE.smokeSeeds); `pnpm ai:gate`
// sets JACKIOH_AI_GATE=full and plays AI_GATE.fullSeeds["ai-vs-random"]. The test passes iff
// wins >= ceil(minWinRate × n), and a failure names the losing seeds, which replay exactly through
// `gameConfig("ai-vs-random", n)`. Every game also has to be clean (B31): nothing rejected, nothing
// thrown, no fallback, a real result, and a log that folds back to the live hash.
