...
- **Search.** Each action is chosen by a turn-level beam search over `legalActions` sequences played through `reduce` on several determinizations. The search starts with an exact lethal solver, answers the AI's prompts the same way, and re-plans after every action. The mulligan returns cards costing more than 3.
