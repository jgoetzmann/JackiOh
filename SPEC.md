...
| Deck library | Server validates and stores | Propose | Up to 100 named decks of at most 20 cards; independent of the loadout (R171) |
...
- Deck library (R171): beside the loadout, an account keeps up to `MAX_LIBRARY_DECKS` (100) named decks, saved one at a time through `/api/decks`. A library deck is checked by the same validator as a single deck (L2, L3, L5, L6; L1 and L4 are loadout rules and do not apply between library decks) and may be saved incomplete. It is re-validated against the current catalog whenever it is used, and importing one into a loadout slot copies its cards. A match may be played with a library deck instead of a loadout deck (R172).
...
**R171 to R173 were added on 2026-09-22** with the deck library (BUILD M9); R173 came from its review. Unlike the rows above they are product decisions rather than rulings found in code, so they are stated before the code that implements them, and the same rule holds: each ships with its `it("R<n> …")` test.
...
| R171 | The deck library | Beside §9.4's one loadout, an account keeps up to `MAX_LIBRARY_DECKS` (100) named decks, each saved on its own; a name is trimmed and holds 1 to `DECK_NAME_MAX_LENGTH` (32) characters. A library deck is validated as a single deck by the shared validator: L3, L5 and L6 hold at save, L2 holds only as a ceiling (a deck of fewer than `DECK_SIZE` cards saves and is shown as incomplete, as Hearthstone saves an unfinished deck), and L1 and L4 do not apply, because they are rules about a loadout and the library is not one: a card may sit in any number of library decks. A deck stores no catalog version; it is re-validated against the current catalog whenever it is used, so a catalog release never invalidates the whole library the way R164 describes for a ban flag. Importing a library deck into a loadout slot copies its cards, and later edits to either do not reach the other. §9.4 describes only the loadout, and the deckbuilder the loadout is prepared in needs somewhere to keep the decks it is not currently using | §9.1, §9.4, R164 |
| R172 | A match may use a library deck | The queue and both room endpoints accept `deckId`, naming one of the caller's own library decks, as an alternative to a loadout `deckIndex`. The deck is validated strictly as a single deck — L2 exactly, L3, L5 and L6 against the current catalog and collection — and frozen into the ticket or room exactly as a loadout deck is (§9.4, §9.8), so editing or deleting it afterwards changes nothing about the match. The loadout is not consulted on this path, so R165 does not apply to it: a profile with no loadout can still play a library deck. Another profile's deck id is not found, never forbidden, so an id reveals nothing | §9.4, §9.5, §9.8, R165 |
| R173 | How large a request body may be | Every API request body is capped at `MAX_REQUEST_BODY_BYTES` (64 KiB) and a larger one is refused with 413 before it is parsed. §9.8 bounds how many requests an account makes (R109) but not how large one is, and the validator reports every failure (§9.4), one issue per bad card id, so a single oversized deck list costs parse and validation time on the process that also runs the match actors and comes back many times larger than it went out. 64 KiB is well over a hundred times the largest legitimate body, a three-deck loadout. The validator's report is bounded the same way, since a body under the cap could still carry thousands of ids: a loadout of more than three decks is reported by L1 alone and a deck of more than `DECK_SIZE` cards by L2 alone, because neither can be judged card by card until it is the right size. A short loadout or deck is still checked in full | §9.4, §9.8, R109 |
