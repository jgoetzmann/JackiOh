...
**R171 and R172 were added on 2026-09-22** with the deck library (BUILD M9). Unlike the rows above they are product decisions rather than rulings found in code, so they are stated before the code that implements them, and the same rule holds: each ships with its `it("R<n> …")` test.
