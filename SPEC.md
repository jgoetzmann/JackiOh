...
- Cards with no listed Radiant form (Quickstriker, Zao Gao, Combo-Fodder, Chaos Golem, My Pawn, and the Rush, Sheep, Felinor and Bread Tokens) are unchanged by becoming Radiant; the flag still sets so counting effects behave.
...
| #68 Twisted Sourcerer | Listed as "Spell, Unit" with stats | Unit |
...
| Tribute X | As an additional cost of playing a card, sacrifice X of your units; a card whose own text tributes (Carnivorous Cube) sacrifices what that text names instead, which may be any of your other permanents, backrow included (R41) | The play-time cost is the play validator's, and the choice travels in the play action (R81); the Sheep Token counts as 2 toward that X while it is on the field (§3.2), and Lava Golem may pick enemy units. A tribute written into a card's script is an ordinary Sacrifice of the permanent that script names, where the Sheep Token's 2 never applies |
...
| --- | --- | --- | --- | --- | --- | --- |
| Rush Token | T-rush | 1 | Unit | 3/3, Rush | none | #15, #58, #60, #74 (X/X), #75, #80, #95 (5/5), #98 |
| Sheep Token | T-sheep | 1 | Unit | 1/1, worth 2 Tributes | none | #41 |
| Felinor Token | T-felinor | 1 | Unit, Felinor | 1/1 | none | #62, #98 |
| Bread Token | T-bread | 0 | Unit | Printed 0/0; always summoned as X/X through `statsOverride` (X = unspent mana, or 3X); no text | none | #18 |
...
- Stat overrides: Adaptive UI summons a Rush Token with X/X (or 3X/3X) instead of 3/3; Call to Chaos summons five as 5/5; Rush Token Farm radiant gives all your Rush Tokens +3/+3 as an aura. Implement as the Rush Token definition plus a `statsOverride` on summon.
...
| 38 | Quickstriker | Epic | 3 | Field Spell |  | Your cards gain "Combo X: deal X damage to the enemy hero", X = cards you played earlier this turn | No radiant form | On each play: damage = `turnLog.cardsPlayed` before this card |
...
| 68 | Twisted Sourcerer | Common | 2 | Unit | 5/5 → 10/10 | Cry: deal 4 damage to a target, 8 if your hero is below 10 | 6, or 12 | Threshold read at resolution |
...
| # | Name | Rarity | Cost | Type, tags | Stats | Base effect | Radiant effect | Engine |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 81 | Radiant Saintess | Epic | 1 | Unit, Human | 2/2 → 4/4 | Cry and Death: all your units become Radiant | Reborn; same | Includes itself on Cry (ruling), so it is 4/4 Reborn immediately; its Reborn body fires Death again |
...
type GameState = {
  ...
  transientDefs: Record<string, CardDef>;  // Fuse and Craft a Card results
...
Each card is one file exporting `{ def, base: Script, radiant: Script }` where `Script = { cost?, cry?, death?, startOfGame?, startOfTurn?, endOfTurn?, aura?, triggers?, activate?, onPlayHook?, handTriggers?, staticFlags?, targets?, modes? }` (`cost` is Ceaseless Void's computed cost, R55; `startOfGame` is Heroic Power's roll, R43) (`targets` and `modes` declare the card's play-time choices, R81) and every hook returns `Effect[]` built from the primitives in section 6. A card never mutates state directly. Tests per card: one fixture per listed behaviour in section 8 (base and radiant), plus a replay test that folds the recorded log and compares the final state hash. Property test: 1,000 random-policy games per seed set must never throw, never desync between two folds, and always terminate within the cap.
...
| R22 | Radiant on the field | Base layer swaps, damage and buffs stay, Cry does not re-fire; Saintess includes itself | #28, #29, #49, #81 |
...
| R119 | A permanent does not answer its own arrival | A permanent whose trigger watches card plays does not fire on the play that put it onto the field: it was not yet in play when that play began, and §10.3 offers an event only to the cards watching when it is dispatched. So a card that copies or reacts to "a card played" starts counting from the next play, not from itself | #33 |
...
| R153 | Which hooks a card answers from each zone | A card registers only the triggers its zone allows. On the field or in the backrow it answers its `triggers` and its `startOfTurn`, `endOfTurn`, `aura`, `setStat` and `onPlayHook` hooks; in a hand it answers only `handTriggers` (#89 Corpse Eater); in a graveyard only the end-of-turn return of a spell that flagged itself when it was played (#23, #24, #31); in a library, in exile, in the resolving zone, or dormant under a Stack it answers nothing (R13). A hook not registered for a zone does not fire from it, so a Field Spell held in hand projects no aura and takes no turn — without this a card summons tokens from the hand it is still sitting in | #58, #64, #89, #23, #24, #31 |
...
| R156 | A state check that begins while a prompt is open | It fires no Death hook. §4.5 steps 1 and 2 still run — the collected cards move and the heroes are checked, because those are the board settling and not a choice — but step 3 is owed in full, in R68's order, and runs when the answer drains `state.work`. Firing a Death hook into an open prompt silently discards whatever that hook asks, because a second prompt can never overwrite an unanswered one; this is the same treatment R62 and R100 give the end-of-turn trap window | §4.5, §9.3, §10.6, R113, R117 |
...
| R169 | The player modifiers travel in the view, on both seats | §10.8 lists what each side of a `PlayerView` carries and does not mention §10.1's `mods`, so a modifier could be active with nothing on screen saying so — which is what happened: BUILD M5-T4 gives `modifierChanged` an animation whose target is `modifiers-<side>`, and no component rendered that element, so #77 Professor Curvature and #78 /fullsend changed the game invisibly. The list travels as `{ id, label }` on **both** seats, because every modifier in the Core set is installed by the Cry of a card played face-up and `modifierChanged` already names the player and the modifier id to both players unredacted; the caption is built from the modifier's own kind and numbers and never from its `sourceId`, so no card identity leaves through a badge (CLAUDE.md rule 7). Per R48 a modifier that covers its controller's *next* turn is installed at once and bites later, so its caption says so while it is not yet live — a badge that claimed #77's discount on the turn the discount does nothing would be worse than no badge | §10.1, §10.3, §10.8, #35, #77, #78, #79, R48, R97 |
